...
// Usage:
//    convert_filter_data input_data_file output_db_file
//
...
void convert_dataset(const char* data_filename, const char* db_filename, int sample_num) 
{
  // Open files
...
int main (int argc, char** argv) 
{
  if (argc != 4) {
    printf("This script converts the text file dataset to the leveldb format used\n"
           "by caffe to perform classification.\n"
           "Usage:\n"
           "   convert_filter_data input_data_file output_db_file sample_num\n");
  } else {
    google::InitGoogleLogging(argv[0]);
    convert_dataset(argv[1], argv[2], atoi(argv[3]));
  }
  return 0;
}
